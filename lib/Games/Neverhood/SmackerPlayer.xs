...
int NHC_VID_Read_Palette(NHC_VID* vid) {
	// int size = NHC_BS_get_8(vid->bs) * 4 - 1;
	// SDL_Palette* current_palette = 
	// int old_colors = vid->surface->format->palette->colors;
	// SDL_Color* old_palette = malloc(sizeof(SDL_Color) * old_colors);
	// int j;
	// for(j = 0; j < old_colors; j++) {
		// memcpy(old_palette[j], vid->surface->for
	// }
	// int sz = 0;
	// int pal_index = 0;
	// while(sz < 256) {
		// Uint8 t = NHC_BS_get_8(vid->bs);
		// if(t & 0x80) {
			// int skip = (t & 0x7F) + 1;
			// sz += skip;
			// pal_index += skip;
